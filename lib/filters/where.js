...
function matchLiteral(model, key, criterion, matchFn, schema) {
  ...
  if (schema[key] && _.isUndefined(val) && _.isNull(criterion)) {
    return true;
  }
    ...
    if (val) {
      switch (schemaType) {
        ...
        case 'date':
          ...
          criterion.setHours(1, 0, 0, 0);
          criterion = criterion.toISOString();
          break;
        case 'time':
          val = new Date(val);
